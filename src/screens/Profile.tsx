...
import { Award, LogOut, Check, Copy, Flame, BookOpen, Shield, CloudLightning } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-6">
        ...
        {user?.synced && (
          <span className="flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-700 text-[10px] font-bold rounded-full border border-emerald-100 uppercase tracking-widest">
      ...
      {/* User Card */}
      <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm mb-6 flex items-center gap-4">
        {user?.photoURL ? (
          <img 
            src={user.photoURL} 
            alt={user.name} 
        ...
        <div className="flex-1 min-w-0">
          <h2 className="font-bold text-slate-900 text-lg truncate leading-snug">{user?.name}</h2>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm text-center">
          ...
          <span className="text-[10px] uppercase tracking-wider text-slate-400 font-extrabold block mb-1">Active Streak</span>
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm text-center">
          ...
          <span className="text-[10px] uppercase tracking-wider text-slate-400 font-extrabold block mb-1">Lessons Done</span>
          <span className="text-xl font-black text-slate-800">{completedLessonsCount} Done</span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm mb-6">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h3 className="font-bold text-slate-900 text-sm">Accountability Buddy</h3>
        </div>
        ...
        <button 
          onClick={handleCopy}
          className="w-full py-4 bg-slate-50 hover:bg-slate-100 text-indigo-600 rounded-2xl font-bold text-xs flex justify-center items-center gap-2 border border-slate-100 transition-all active:scale-95"
        >
      ...
      <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm mb-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h4 className="font-bold text-slate-900 text-sm">Unlocked Rewards</h4>
        </div>
        
        {badges.length === 0 ? (
          <p className="text-xs text-slate-400 text-center py-4 font-medium leading-relaxed">
          ...
          <div className="space-y-3">
            {badges.map((badgeName) => (
              ...
              >
                <div className="w-8 h-8 bg-amber-50 text-amber-500 flex items-center justify-center text-sm rounded-xl">
                  🏆
                </div>
                <div>
                  <p className="text-xs font-bold text-slate-800">{badgeName}</p>
                  <p className="text-[10px] text-slate-400 font-medium">Unlocked through communication streak</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-rose-50 hover:bg-rose-100 text-rose-600 font-bold rounded-2xl text-xs transition-colors shadow-sm active:scale-95 border border-rose-100"
      >
